-- Cleanup migration: remove unsuffixed Berlin deployment rows now that suffixed rows exist.
-- Behavior: validate target *_berlin rows exist, rewire any leftover child references,
-- then delete old unsuffixed rows. It does NOT overwrite target deployment content.
...
DECLARE
    ...
    table_row record;
BEGIN
    CREATE TEMP TABLE tmp_cleanup_berlin_unsuffixed_map (
    ...
    INSERT INTO tmp_cleanup_berlin_unsuffixed_map (
        old_deployment_id,
        new_deployment_id,
        old_tune_id,
        new_tune_id,
    ...
    FROM tmp_cleanup_berlin_unsuffixed_map;

    IF map_count = 0 THEN
        RAISE NOTICE 'cleanup_unsuffixed_berlin_deployments: no unsuffixed berlin rows found';
        RETURN;
    END IF;
    ...
    IF EXISTS (
        SELECT 1
        FROM tmp_cleanup_berlin_unsuffixed_map
        WHERE old_deployment_id = new_deployment_id
    ) THEN
        RAISE EXCEPTION 'cleanup_unsuffixed_berlin_deployments: invalid deployment_id format for one or more rows';
    END IF;

    -- Every unsuffixed row must already have a matching suffixed target row.
    SELECT COUNT(*)::bigint INTO missing_target_rows
    FROM tmp_cleanup_berlin_unsuffixed_map m
    ...
    IF missing_target_rows > 0 THEN
        RAISE EXCEPTION 'cleanup_unsuffixed_berlin_deployments: missing *_berlin target rows (%)', missing_target_rows;
    END IF;

    -- Target row identity must match expected symbol/strategy/tune.
    ...
    IF incompatible_target_rows > 0 THEN
        RAISE EXCEPTION 'cleanup_unsuffixed_berlin_deployments: incompatible *_berlin target rows found (%)', incompatible_target_rows;
    END IF;

    -- Unique triplet guard: no third deployment row may already own the target triplet.
    ...
    WHERE d.deployment_id <> m.old_deployment_id
      AND d.deployment_id <> m.new_deployment_id;

    IF triplet_conflicts > 0 THEN
        RAISE EXCEPTION 'cleanup_unsuffixed_berlin_deployments: target (symbol,strategy_id,tune_id) conflicts found (%)', triplet_conflicts;
    END IF;

    -- Prevent unique-key collisions while moving child rows.
    DELETE FROM scalp_sessions old_s
    USING tmp_cleanup_berlin_unsuffixed_map m, scalp_sessions new_s
    ...
    USING tmp_cleanup_berlin_unsuffixed_map m, scalp_execution_runs new_r
    ...
    USING tmp_cleanup_berlin_unsuffixed_map m, scalp_deployment_weekly_metrics new_w
    ...
    -- Remove old unsuffixed deployment rows.
    DELETE FROM scalp_deployments d
    USING tmp_cleanup_berlin_unsuffixed_map m
    WHERE d.deployment_id = m.old_deployment_id;

    RAISE NOTICE 'cleanup_unsuffixed_berlin_deployments: cleaned % deployment(s)', map_count;
END $$;
