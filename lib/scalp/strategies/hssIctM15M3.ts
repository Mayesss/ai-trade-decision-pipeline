...
function applyPhaseDetectors(
  params: ScalpStrategyPhaseInput,
): ScalpStrategyPhaseOutput {
  ...
  if (
    !next.sweep &&
    params.nowMs > params.windows.raidEndMs &&
    next.state === "ASIA_RANGE_READY"
  ) {
    next.state = "DONE";
  ...
  if (next.state === "ASIA_RANGE_READY" || next.state === "SWEEP_DETECTED") {
    const atrBase = computeAtr(
      params.market.baseCandles,
    ...
    const sweep = detectSweepLifecycle({
      ...
      pipSize: pipSizeForScalpSymbol(next.symbol, params.market.symbolMeta),
  ...
  if (next.state === "CONFIRMING") {
    ...
    if (
      ...
      confirmation.displacementTsMs &&
      confirmation.structureShiftTsMs
    ) {
      ...
      if (!ifvg.zone) {
        if (
          params.nowMs >
          rejectionTsMs + params.cfg.confirm.ttlMinutes * 60_000
        ) {
          next.state = "DONE";
