...
function withLastProcessed(
  state: ScalpSessionState,
  input: ScalpStrategyPhaseInput["market"],
): ScalpSessionState {
...
function bucketCandlesByDay(
  candles: ScalpCandle[],
  timeZone: string,
): Map<string, ScalpCandle[]> {
...
function expectedDirectionFromSweep(
  state: ScalpSessionState,
): ScalpDirectionalBias | null {
...
function buildReferenceRange(params: {
  ...
  const currentDayCandles = (buckets.get(params.state.dayKey) || [])
    .slice()
    .sort((a, b) => ts(a) - ts(b));
  ...
  const referenceCandles = (buckets.get(referenceDayKey) || [])
    .slice()
    .sort((a, b) => ts(a) - ts(b));
  ...
  if (
    !(
      Number.isFinite(referenceHigh) &&
      Number.isFinite(referenceLow) &&
...
function buildCurrentDaySweepWindow(
  windows: ScalpSessionWindows,
  currentDayCandles: ScalpCandle[],
): ScalpSessionWindows {
...
function applyPhaseDetectorsWithOptions(
  ...
  if (next.state === "ASIA_RANGE_READY" || next.state === "SWEEP_DETECTED") {
    ...
    const sweep = detectSweepLifecycle({
      ...
      pipSize: pipSizeForScalpSymbol(next.symbol, input.market.symbolMeta),
      cfg: input.cfg.sweep,
    });
    ...
    reasonCodes.push(
      sweep.direction === "BULLISH"
        ? "PDH_PDL_SELL_SIDE_RECLAIM"
        : "PDH_PDL_BUY_SIDE_RECLAIM",
    );
  }
  ...
  if (next.state === "CONFIRMING") {
    ...
    if (confirmation.displacementTsMs && confirmation.structureShiftTsMs) {
      ...
      if (!ifvg.zone) {
        if (
          input.nowMs >
          rejectionTsMs + Math.max(1, input.cfg.confirm.ttlMinutes) * 60_000
        ) {
          next.state = "DONE";
