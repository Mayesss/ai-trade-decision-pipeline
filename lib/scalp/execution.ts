...
export function buildScalpDealReference(params: {
  deploymentId: string;
  setupId: string;
  dayKey: string;
}): string {
  const suffix = hashForSetup(`${params.setupId}:${params.dayKey}`).slice(
    0,
    12,
  );
...
export function matchesScalpDeploymentDealReference(
  ...
  return normalized.startsWith(
    scalpDeploymentDealReferencePrefix(deploymentId),
  );
}

export function resolveLegacyIfvgEntryIntent(
  state: ScalpSessionState,
): ScalpStrategyEntryIntent | null {
...
function currentRForTrade(
  trade: NonNullable<ScalpSessionState["trade"]>,
  price: number,
  riskAbs: number,
): number {
  if (!(Number.isFinite(price) && Number.isFinite(riskAbs) && riskAbs > 0))
    return 0;
  const signedMove =
...
async function closeScalpTradePortion(params: {
  ...
  const brokerPositionId =
    String(
      params.trade.brokerPositionId ?? params.trade.brokerOrderId ?? "",
    ).trim() || null;
    ...
    return {
      ...
      reasonCodes: [
        ...
        err instanceof Error
          ? err.message.toUpperCase().slice(0, 80)
          : "TRADE_CLOSE_UNKNOWN_ERROR",
      ],
    };
  }
}
...
function tradeBrokerPositionId(
  ...
  const brokerPositionId = String(
    trade?.brokerPositionId ?? trade?.brokerOrderId ?? "",
  ).trim();
...
function resolveOwnedScalpBrokerMatches(params: {
  ...
  const sameEpic = params.snapshots.filter(
    (row) => String(row.epic || "").trim() === params.epic,
  );
  const brokerPositionId = tradeBrokerPositionId(params.trade);
  if (brokerPositionId) {
    const matches = sameEpic.filter(
      (row) => String(row.dealId || "").trim() === brokerPositionId,
    );
  ...
  if (dealReference) {
    const matches = sameEpic.filter(
      (row) => String(row.dealReference || "").trim() === dealReference,
    );
    if (matches.length > 0)
      return { matches, matchedBy: "dealReference", sameEpic };
  }

  const matches = sameEpic.filter((row) =>
    matchesScalpDeploymentDealReference(row.dealReference, params.deploymentId),
  );
...
export async function reconcileScalpBrokerPosition(params: {
  ...
  if (params.dryRun)
    return {
      state: params.state,
      reasonCodes: ["BROKER_RECONCILE_SKIPPED_DRY_RUN"],
    };
  ...
  if (!params.snapshots) {
    if (params.skipSnapshotFetch) {
      return {
        state: params.state,
        reasonCodes: ["BROKER_RECONCILE_UNAVAILABLE"],
      };
    }
    try {
  ...
  if (owned.matches.length > 1) {
    ...
    return {
      state: next,
      reasonCodes: [
        owned.matchedBy === "deployment"
          ? "BROKER_DEPLOYMENT_POSITION_AMBIGUOUS"
          : "BROKER_OWNED_POSITION_AMBIGUOUS",
      ],
    };
  }
  ...
  const side =
    ownedPosition.side === "long"
      ? "BUY"
      : ownedPosition.side === "short"
        ? "SELL"
        : null;
  ...
  if (next.trade && !next.trade.dryRun) {
    next.trade.dealReference =
      String(
        ownedPosition.dealReference || next.trade.dealReference || "",
      ).trim() || next.trade.dealReference;
    next.trade.brokerOrderId =
      String(ownedPosition.dealId || next.trade.brokerOrderId || "").trim() ||
      null;
    next.trade.brokerPositionId =
      String(
        ownedPosition.dealId ||
          next.trade.brokerPositionId ||
          next.trade.brokerOrderId ||
          "",
      ).trim() || null;
  ...
  next.trade = {
    ...
    dealReference: String(
      ownedPosition.dealReference || `recovered-${next.deploymentId}`,
    ),
    side,
    entryPrice,
    ...
    initialStopPrice:
      next.trade?.initialStopPrice ?? next.trade?.stopPrice ?? entryPrice,
...
export async function manageScalpOpenTrade(params: {
  ...
  if (!next.trade) {
    return {
      state: next,
      reasonCodes: ["TRADE_MANAGE_SKIPPED_NO_TRADE"],
      closedTrade: null,
    };
  }
  ...
  if (
    ...
    return {
      state: next,
      reasonCodes: ["TRADE_MANAGE_INVALID_PRICING"],
      closedTrade: null,
    };
  }

  const inferredRiskAbs = Math.abs(
    entry - toFinite(trade.initialStopPrice ?? trade.stopPrice),
  );
  const riskAbs = toFinite(trade.riskAbs, inferredRiskAbs);
  if (!(Number.isFinite(riskAbs) && riskAbs > 0)) {
    return {
      state: next,
      reasonCodes: ["TRADE_MANAGE_INVALID_RISKABS"],
      closedTrade: null,
    };
  }
  ...
  trade.tp1Price = Number.isFinite(toFinite(trade.tp1Price, NaN))
    ? Number(trade.tp1Price)
    : null;
  trade.trailStopPrice = Number.isFinite(toFinite(trade.trailStopPrice, NaN))
    ? Number(trade.trailStopPrice)
    : null;
  trade.favorableExtremePrice = Number.isFinite(
    toFinite(trade.favorableExtremePrice, NaN),
  )
  ...
  const barsHeld = Math.max(
    0,
    Math.floor((params.nowMs - trade.openedAtMs) / Math.max(60_000, tfMs)),
  );
  ...
  const atrAbs = computeAtr(
    params.market.confirmCandles,
    params.cfg.data.atrPeriod,
  );

  if (
    !trade.tp1Done &&
    currentR >= params.cfg.risk.tp1R &&
    trade.remainingSizePct > 0
  ) {
    ...
    if (tp1Pct > 0) {
      ...
      if (tp1Close.closed) {
        const closedFraction = closeFractionFromPct(
          trade.remainingSizePct,
          tp1Pct,
        );
        trade.remainingSizePct = Math.max(
          0,
          trade.remainingSizePct - closedFraction,
        );
        trade.realizedR += closedFraction * params.cfg.risk.tp1R;
        trade.tp1Done = true;
        trade.tp1Price = price;
        const beOffsetAbs =
          Math.max(0, params.cfg.risk.breakEvenOffsetR) * riskAbs;
        const beStop =
          trade.side === "BUY" ? entry + beOffsetAbs : entry - beOffsetAbs;
        trade.stopPrice =
          trade.side === "BUY"
            ? Math.max(trade.stopPrice, beStop)
            : Math.min(trade.stopPrice, beStop);
  ...
  if (totalTradeR > 0) {
    ...
    next.stats.losses += 1;
    next.stats.consecutiveLosses =
      Math.max(0, next.stats.consecutiveLosses) + 1;
  } else {
    next.stats.consecutiveLosses = 0;
  }
  ...
  if (next.stats.realizedR <= params.cfg.risk.dailyLossLimitR) {
    ...
    totalTradeR < 0 &&
    next.stats.consecutiveLosses >=
      Math.max(1, params.cfg.risk.consecutiveLossPauseThreshold)
  ) {
    const cooldownBars = Math.max(
      0,
      params.cfg.risk.consecutiveLossCooldownBars,
    );
    if (cooldownBars > 0) {
      next.cooldownUntilMs =
        params.nowMs + cooldownBars * Math.max(60_000, tfMs);
      next.state = "COOLDOWN";
