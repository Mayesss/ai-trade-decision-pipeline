...
import { applyScalpStrategyConfigOverride, getScalpStrategyConfig } from '../config';
...
function marketSnapshot(params: { nowMs: number; price: number; confirmCandles: ScalpCandle[] }): ScalpMarketSnapshot {
    return {
        symbol: 'EURUSD',
        epic: 'REPLAY:EURUSD',
        nowMs: params.nowMs,
...
test('manageScalpOpenTrade closes position on stop hit and records loss stats', async () => {
    ...
    assert.ok((managed.state.stats.realizedR ?? 0) < 0);
