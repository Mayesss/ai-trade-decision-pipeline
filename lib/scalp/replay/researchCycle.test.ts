...
test("buildResearchCycleTasks chunks lookback windows across allowed strategies", () => {
  ...
  const strategyIds = Array.from(
    new Set(tasks.map((task) => task.strategyId)),
  ).sort();
...
test("buildResearchCycleTasks expands symbol+strategy into capped tune variants when tuner is enabled", () => {
  ...
  assert.ok(
    tasks.some(
      (row) => row.configOverride && Object.keys(row.configOverride).length > 0,
    ),
  );
});

test("buildResearchCycleTasks skips historical windows already completed or aborted", () => {
  const nowMs = Date.UTC(2026, 2, 2);
  const completedWindow: ScalpResearchTask = {
    ...makeTask({
      cycleId: "rc_history",
      taskId: "hist_completed",
      symbol: "BTCUSDT",
  ...
  const abortedWindow: ScalpResearchTask = {
    ...makeTask({
      cycleId: "rc_history",
      taskId: "hist_aborted",
      symbol: "BTCUSDT",
    ...
    deploymentId: "BTCUSDT~regime_pullback_m15_m3~default",
    tuneId: "default",
    windowFromTs: Date.UTC(2026, 1, 9),
    windowToTs: Date.UTC(2026, 1, 16),
  };

  const tasks = buildResearchCycleTasks({
    cycleId: "rc_skip_history",
    nowMs,
    symbols: ["BTCUSDT"],
    lookbackDays: 28,
    chunkDays: 7,
    ...
    tunerEnabled: false,
    maxTuneVariantsPerStrategy: 1,
    historicalTasks: [completedWindow, abortedWindow],
  });
...
test("summarizeResearchTasks aggregates candidate metrics and keeps running status with pending tasks", () => {
  ...
  const guarded = summary.candidateAggregates.find(
    (row) =>
      row.symbol === "BTCUSDT" && row.strategyId === "regime_pullback_m15_m3",
  );
  assert.ok(guarded);
...
test("summarizeResearchTasks treats aborted tasks as terminal failures", () => {
  const cycle = makeCycle("rc_sum_aborted");
  const completed = makeTask({
    cycleId: cycle.cycleId,
    taskId: "t_ok",
    symbol: "BTCUSDT",
  ...
  const summary = summarizeResearchTasks(cycle, [completed, aborted]);
  const candidate = summary.candidateAggregates.find(
  ...
  assert.equal(summary.progressPct, 100);
  assert.ok(candidate);
  assert.equal(candidate.failedTasks, 1);
});
...
test("summarizeResearchTasks keeps tune variants as separate candidates", () => {
  ...
  assert.ok(
    summary.candidateAggregates.some((row) => row.tuneId === "default"),
  );
  assert.ok(
    summary.candidateAggregates.some((row) => row.tuneId === "auto_tp30"),
  );
});
...
test("evaluateResearchTaskClaimability blocks stale running reclaim when max attempts are exhausted", () => {
  ...
  assert.equal(out.shouldAbortForAttempts, true);
});
...
test("symbol cooldown activates after repeated eligible failures in window", () => {
  ...
  assert.equal(
    isResearchSymbolCooldownActive(snapshot, "EURUSD", nowMs + 10_000),
    true,
  );
  assert.equal(
    isResearchSymbolCooldownActive(snapshot, "EURUSD", c.blockedUntilMs + 1),
    false,
  );
});
...
test("evaluateResearchCyclePreflight excludes underfilled symbols but allows ready symbols", async (t) => {
  ...
  try {
    ...
    process.env.SCALP_SYMBOL_UNIVERSE_PATH = path.join(
      tmpRoot,
      "universe.json",
    );
    ...
    const startCurrentWeekMondayMs =
      Date.UTC(2026, 2, 11, 0, 0, 0) - daysSinceMonday * oneDayMs;
    ...
    assert.ok(
      out.candleChecks.some(
        (row) => row.symbol === "GBPUSD" && row.ok === false,
      ),
    );
    assert.equal(
      out.failures.some((row) => row.code === "insufficient_candles"),
      false,
    );
  } finally {
    if (prevEnv.CANDLE_HISTORY_DIR === undefined)
      delete process.env.CANDLE_HISTORY_DIR;
    else process.env.CANDLE_HISTORY_DIR = prevEnv.CANDLE_HISTORY_DIR;
    if (prevEnv.SCALP_SYMBOL_UNIVERSE_PATH === undefined)
      delete process.env.SCALP_SYMBOL_UNIVERSE_PATH;
    else
      process.env.SCALP_SYMBOL_UNIVERSE_PATH =
        prevEnv.SCALP_SYMBOL_UNIVERSE_PATH;
    if (prevEnv.SCALP_RESEARCH_REPORT_PATH === undefined)
      delete process.env.SCALP_RESEARCH_REPORT_PATH;
    else
      process.env.SCALP_RESEARCH_REPORT_PATH =
        prevEnv.SCALP_RESEARCH_REPORT_PATH;
    if (prevEnv.ALLOW_SCALP_FILE_BACKEND === undefined)
      delete process.env.ALLOW_SCALP_FILE_BACKEND;
    else
      process.env.ALLOW_SCALP_FILE_BACKEND = prevEnv.ALLOW_SCALP_FILE_BACKEND;
...
test("evaluateResearchCyclePreflight filters symbols missing 12 successive completed weeks", async (t) => {
  ...
  const prevEnv = {
    ...
    SCALP_SYMBOL_UNIVERSE_PATH: process.env.SCALP_SYMBOL_UNIVERSE_PATH,
    SCALP_RESEARCH_REPORT_PATH: process.env.SCALP_RESEARCH_REPORT_PATH,
    SCALP_RESEARCH_PREFLIGHT_REQUIRED_SUCCESSIVE_WEEKS:
  ...
  try {
    ...
    const makeWeeklyCandles = (weekIndexes: number[]) =>
      weekIndexes.map((week) => {
        const ts =
          firstRequiredWeekStartMs + week * oneWeekMs + 12 * 60 * 60 * 1000;
        return [ts, 1.1, 1.11, 1.09, 1.105, 10] as [
          number,
          number,
          number,
          number,
    ...
    const gbpExcluded = (
      out.weeklySuccessiveRequirement?.excludedSymbols || []
    ).find((row) => row.symbol === "GBPUSD");
    if (gbpExcluded) {
    ...
    if (prevEnv.SCALP_RESEARCH_REPORT_PATH === undefined)
      ...
      process.env.SCALP_RESEARCH_REPORT_PATH =
        prevEnv.SCALP_RESEARCH_REPORT_PATH;
    if (
      prevEnv.SCALP_RESEARCH_PREFLIGHT_REQUIRED_SUCCESSIVE_WEEKS === undefined
    )
...
test("evaluateResearchCyclePreflight enforces lookback-derived week floor over lower configured requirement", async (t) => {
  ...
  try {
    ...
    const candles = Array.from({ length: 12 }, (_, idx) => {
      const ts =
        firstRequiredWeekStartMs + (idx + 1) * oneWeekMs + 12 * 60 * 60 * 1000;
      return [ts, 1.1, 1.11, 1.09, 1.105, 10] as [
        number,
        number,
        number,
    ...
    assert.equal(
      out.failures.some((row) => row.code === "no_symbols_eligible"),
      true,
    );
  } finally {
    if (prevEnv.CANDLE_HISTORY_DIR === undefined)
