  ...
  assert.equal(
    shouldPruneResearchCycle({
      cycle: {
        cycleId: "rc_a",
        status: "completed",
        updatedAtMs: nowMs - retentionMs - 1,
      },
      nowMs,
      activeCycleId: "rc_a",
      retentionMs,
  ...
  assert.equal(
    shouldPruneResearchCycle({
      cycle: {
        cycleId: "rc_running2",
        status: "running",
        updatedAtMs: nowMs - 2 * retentionMs - 1,
      },
      nowMs,
      activeCycleId: null,
      retentionMs,
...
test("pruneScalpCandlesToRollingWeeks keeps only 12 Monday-Sunday weeks when new week starts", () => {
  ...
  const out = pruneScalpCandlesToRollingWeeks({
    candles: candles.map(
      (row) => [...row] as [number, number, number, number, number, number],
    ),
    nowMs,
    keepWeeks: 12,
  });
...
test("resolveScalpResearchHistoryRetentionPolicy keeps 12-week primary reference and 52-week confirmation history by default", () => {
  const out = resolveScalpResearchHistoryRetentionPolicy({
    primaryReferenceWeeks: 12,
    confirmationKeepWeeks: 52,
    confirmationBufferWeeks: 2,
  });

  assert.equal(out.primaryReferenceWeeks, 12);
  assert.equal(out.confirmationKeepWeeks, 52);
  assert.equal(out.confirmationBufferWeeks, 2);
  assert.equal(out.minimumRetainedWeeks, 54);
  assert.equal(out.minimumCycleRetentionDays, 385);
});

test("resolveScalpResearchHistoryRetentionPolicy never lets confirmation history shrink below primary reference horizon", () => {
  const out = resolveScalpResearchHistoryRetentionPolicy({
    primaryReferenceWeeks: 20,
    confirmationKeepWeeks: 12,
    confirmationBufferWeeks: 1,
  });

  assert.equal(out.primaryReferenceWeeks, 20);
