...
// When flat, only spend an AI call if the code-computed signal_strength is at
// least this rank. LOW setups can't open a quality position anyway, so we skip
// the call and HOLD. Entries are evaluated hourly (data is collected every tick);
// the AI only fires when a setup is actually actionable. In-position ticks always
// call the AI regardless (exits/trims can be needed at any strength).
export const SWING_FLAT_MIN_SIGNAL_STRENGTH: 'MEDIUM' | 'HIGH' = 'MEDIUM';
