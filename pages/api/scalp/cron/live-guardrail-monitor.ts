...
import { runScalpLiveGuardrailMonitor } from '../../../../lib/scalp/liveGuardrail';
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    const autoPause = parseBoolParam(req.query.autoPause, true);
    ...
    try {
        const out = await runScalpLiveGuardrailMonitor({
            dryRun,
            autoPause,
            tradeLimit,
            monthlyMonths,
        ...
        return res.status(500).json({
            error: 'live_guardrail_monitor_failed',
